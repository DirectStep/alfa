      ...
      <Container>
        ...
        <ul className="mt-8 grid grid-cols-1 gap-5 md:grid-cols-2 laptop:grid-cols-3">
          {benefitCards.map((card) => (
            <li
              key={card.id}
              className={`group relative flex min-h-[440px] flex-col justify-between overflow-hidden rounded-[30px] p-7 transition-transform duration-300 hover:-translate-y-1 laptop:min-h-[460px] ${variantClasses[card.variant]}`}
            >
